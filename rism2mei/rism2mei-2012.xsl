...
<!--
	...
	rism2mei-2012.xsl - XSLT (1.0) stylesheet for transformation of RISM MARC XML to MEI header XML
...
<xsl:stylesheet version="1.0" 
  ...
  <xsl:param name="header_only">false</xsl:param>
	<!-- true | false -->
  <xsl:param name="rng_model">true</xsl:param>
  <!-- agency name -->
  <xsl:param name="rng_model_path">href="/Users/laurent/Documents/mei/2012/mei-all.rng"</xsl:param>
  <!-- agency name -->
	...
	<xsl:template name="mei">
	  <xsl:choose>
	    <xsl:when test="$rng_model = 'true'">
	      <xsl:processing-instruction name="xml-model">
	        <xsl:value-of select="$rng_model_path"/>
	...
	<xsl:template
		...
		<annot type="{$annottype}" analog="marc:{$tag}">
			<xsl:choose>
	...
	<xsl:template match="marc:datafield[@tag='594']">
		<xsl:variable name="tag" select="@tag"/>
		<annot type="scoring" analog="marc:{$tag}">
	...
	<xsl:template name="source_notesStmt">
		...
		<xsl:choose>
			<xsl:when test="$tag='590' and $node/marc:subfield[@code='a']">
				<annot type="parts" analog="marc:{$tag}">
					<xsl:call-template name="subfieldSelect">
						<xsl:with-param name="codes">a</xsl:with-param>
					</xsl:call-template>
				</annot>
				...
				<annot type="missing_parts" analog="marc:{$tag}">
					<xsl:call-template name="subfieldSelect">
						<xsl:with-param name="codes">b</xsl:with-param>
					</xsl:call-template>
				</annot>
			...
			<xsl:when test="$tag='592'">
				<annot type="watermark" analog="marc:{$tag}">
					<xsl:call-template name="subfieldSelect">
						<xsl:with-param name="codes">a</xsl:with-param>
					</xsl:call-template>
				</annot>
			...
			<xsl:when test="$tag='593'">			
				<annot type="source_type" analog="marc:{$tag}">
					<xsl:call-template name="subfieldSelect">
						<xsl:with-param name="codes">a</xsl:with-param>
					</xsl:call-template>
				</annot>
	...
	<xsl:template
		...
		<term classcode="{$classcode}" analog="marc:{$tag}">
			<xsl:call-template name="subfieldSelect">
				<xsl:with-param name="codes">a</xsl:with-param>
			</xsl:call-template>
		</term>
		...
		<section n="{./marc:subfield[@code='a']}">
			<staff n="1">
				<layer>
				  ...
				  <xsl:copy-of select="document('file:///Users/laurent/test.xml')//svg:defs"/>
				  <!--xsl:apply-templates select="document('file:///Users/laurent/test.xml')"/-->
				</layer>
			</staff>
		</section>
	</xsl:template>
